...
The main money amount should let the user choose available actions for adding money, subtracting money, or modifying the full money amount. The available actions should depend on whether the money amount is `0.00$`, between `0.00$` and `999,999.99$`, or exactly `999,999.99$`.

The website should make the main money actions easy to understand without using real banking language.

### Open UI Priority

Only one temporary UI should be open at a time so the user does not have two unfinished actions open at once.

If a temporary UI is open, the user should close it, cancel it, save it, delete from it, finish it, or drop/cancel it before another temporary UI can open.

If the user refreshes the page, closes the browser tab or window, or reopens the website later while any temporary UI is open, the temporary UI should be discarded and the next load should show the latest successfully saved data.

### Storage Scope

The first version should save the user's data only in browser storage.

The first version should not use email accounts, login, cloud sync, server storage, or a database.

If saved browser data is broken or cannot be read, the website should give the user a way to start again from fresh saved data.

In the first version, normal saved data should not have a user-controlled reset, clear-all-data, or start-fresh action. The `Start again` action should be shown only when saved browser data is broken or cannot be read.
...
Main money amount actions should not make the main money amount go above `999,999.99$`.
...
The `Balance Changes` area should appear directly under the main money amount, and the dashboard page space under the main money amount should be mainly dedicated to this area.
...
`Balance Changes` entries should be ordered newest first.
...
`Modify` should silently correct the money amount without creating a `Balance Changes` entry, activity entry, or notification entry.
...
The full-screen `Savings` view should show the `Savings money amount` and the `Saving` squares.

The money amount shown inside `Savings` should still start from the main money amount and go to the visible `Saving` squares from top to bottom as planning display only.

Visible `Saving` squares should appear in one vertical column on mobile and desktop. The website should not lay out `Saving` squares in multiple columns or a grid.

In the first version, `Saving` squares should not save or show a created date or updated date.

The user can add, rename, change the planned money amount, reorder, fix, and delete `Saving` squares.
...
When the total planned money amount in valid `Saving` squares is greater than the main money amount, the top `Savings` area should show a small text note directly under the `Savings money amount` using the exact format `{money amount} needed`, such as `20.00$ needed`.
...
`Saving` squares should have an order. The first `Saving` square the user creates should stay at the top by default.

The visible order of normal `Saving` squares is the coverage order. Broken `Saving` squares should be skipped.

Reordering `Saving` squares should not change the main money amount and should not create a `Balance Changes` entry.

When the user deletes a `Saving` square, the website should ask the user to confirm the delete action before deleting the `Saving` square.

Deleting a `Saving` square should remove only that `Saving` square and the details inside it. It should not change the main money amount or create a `Balance Changes` entry.
...
- In the first version, clickable controls should be designed for mouse clicks and finger taps.
...
If the main money action buttons are visible and the user scrolls the dashboard page, the buttons should go away without changing the money amount, saving anything, creating a `Balance Changes` entry, or showing a message.
...
Using the browser Back button, mobile browser back gesture, or system Back action while an `Add`, `Subtract`, or `Modify` main money amount input flow is open should act like `Cancel`: the horizontal input square should close, the dashboard money amount view should return, nothing should change, nothing should save, no `Balance Changes` entry should be created, and no message should appear.

Refreshing the page, closing the browser tab or window, or reopening the website later while an `Add`, `Subtract`, or `Modify` main money amount input flow has unsaved typed input should discard that open input flow and typed value. The next website load should show the last successfully saved data, change nothing, save nothing, create no `Balance Changes` entry, and show no message or browser leave warning.
...
### Balance Changes Interaction Details
...
Clicking `Cancel` in the delete confirmation should close the confirmation without deleting the entry, changing the main money amount, saving anything, creating a `Balance Changes` entry, or showing a message.

Clicking or tapping outside the delete confirmation should close the confirmation without deleting the entry, changing the main money amount, saving anything, creating a `Balance Changes` entry, or showing a message.
...
The top square in the column is checked first for coverage, then the next square below it, continuing down the column.
...
The `Saving` square delete confirmation should appear as a small confirmation square in the middle of the screen. It should show the exact message `Delete this Saving?` and buttons exactly named `Cancel` and `Delete`. It should not appear inside the `Saving` square, as a temporary square state, as a bottom sheet, or as a separate page. Only one `Saving` square delete confirmation should be open at a time.

Opening the `Saving` square delete confirmation from a normal `Saving` square should clear that square's action state. Clicking `Cancel` or clicking or tapping outside the small confirmation square should close the confirmation, return the selected normal `Saving` square to its default state, change nothing, save nothing, create no `Balance Changes` entry, and show no message.

Clicking `Delete` inside the small confirmation square should close the confirmation and delete only the selected `Saving` square using the normal delete rules.

Opening rename for an existing `Saving` square should replace that same square with a temporary `Saving` input square in the same visible position. The rename input square should ask only for the new `Saving` name and should show the bottom text actions `Save` and `Cancel`. It should not appear as a modal, bottom sheet, separate page, or separate floating input.

Opening planned-money-amount change for an existing `Saving` square should replace that same square with a temporary `Saving` input square in the same visible position. The planned-money-amount change input square should ask only for the new total planned money amount and should show the bottom text actions `Save` and `Cancel`. It should not appear as a modal, bottom sheet, separate page, or separate floating input.
...
If the user tries to create, rename, or fix a `Saving` square with a duplicate name, the website should not save that duplicate name.

If the user enters a duplicate `Saving` name while creating, renaming, or fixing a `Saving` square, the website should return to the `Saving` squares view without showing a duplicate-name error message. Nothing should change: no new `Saving` square should be created, an existing `Saving` square should keep its old name, the broken `Saving` square should stay broken when a fix name is duplicate, saved data should stay unchanged, and `Balance Changes` should not get a new entry.
...
### Open UI Interaction Details

Temporary UI means main money action buttons, a main money amount input flow, a `Balance Changes` delete action square, a `Balance Changes` delete confirmation, a `Saving` square action state, a `Saving` square input flow, a `Saving` square delete confirmation, or an active `Saving` square reorder drag.

Full-screen `Savings` should count as the current view while it is open, not as a temporary UI layer. Opening full-screen `Savings` from the dashboard should be allowed only when no dashboard temporary UI is open.

A click, tap, or hold that would normally open a different temporary UI should not open that new temporary UI while another one is already open.

If the same click or tap is already defined as an outside-click close or outside-click cancel for the open temporary UI, it should only close or cancel the open temporary UI. It should not also activate the thing behind it or open a different temporary UI from that same click or tap.
...
## Belongs to the Technical Spec

The items in this section are still inside the business spec for review, but they look more like storage, validation, data model, implementation, or HTML/CSS detail rules than business-level requirements.
...
### HTML CSS informations required
