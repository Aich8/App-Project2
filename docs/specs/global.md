...
Main money action inputs for `Add`, `Subtract`, and `Modify` should open as a horizontal money amount input square in the middle of the screen, with the main money amount circle still visible in the background. The square should start by showing `0.00` without the `$` sign. The user should type only the money amount, without the `$` sign. If the typed money amount reaches `1,000.00` or more, the input should add comma separators automatically while the user is typing. Main money amount inputs should request a decimal numeric keyboard on supported mobile devices, so the user gets number keys `0` through `9` and a decimal point, the same as `Saving` square planned money amount inputs. Main money amount inputs should accept numbers from `0` through `9`, one decimal point for cents, and the `Space` key as either a starting zero-position skip before a nonzero digit or a cents-position skip after at least one accepted digit. The first accepted non-`Space` character should be a number from `0` through `9`, the decimal point may appear only once, and the user should not be able to type more than two digits after the decimal point. The user should not manually type comma separators into the input. If a typed character would make the displayed input money amount greater than `999,999.99`, the field should not change and no message should appear. While the user is typing, the website should always show the input value with two digits after the decimal point, automatic comma separators for thousands and larger values, and no `$` sign, such as `5.00`, `5.05`, `5,895.50`, or `999,999.99`. For large cents examples, the user types the raw digit sequence before formatting: typing `589550` should show `5,895.50`, and typing `99999999` should show `999,999.99`. After saving, the saved and displayed money amount should show comma separators for thousands and larger values and the `$` sign at the end, such as `5.00$`, `5.05$`, `5,895.50$`, or `999,999.99$`. At the start of the input, `0` and `Space` should both count as zero-position skips until the user types a nonzero digit. The user may enter at most three starting zero-position skips before a nonzero digit. A fourth starting zero-position skip should be blocked with no message. If the user saves after entering only starting zero-position skips, the typed value should be treated as `0.00` and follow the existing zero-input rules. Typing `0005` or typing `Space`, `Space`, `Space`, then `5` should show `00.05` in the input and save/display as `00.05$`. Typing `0.05` should show `0.05` in the input and save/display as `0.05$`. If the user types `5`, then `Space`, then `Space`, then `5`, skipped cents positions should stay as `0`, so the input should show `5.05` and the saved/displayed money amount should be `5.05$`. The `Space` key should not insert visible blank spaces into the input or saved money amount. If the user deletes one typed character, the remaining typed value should be formatted again with two digits after the decimal point, automatic comma separators when needed, and no `$` sign. If the user deletes all typed numbers, the square should return to `0.00` and should not become empty. Blocked typed characters, including manually typed comma separators, should not appear in the field, and no message should appear for those blocked typed characters. The user should not be able to paste into main money action inputs. If an `Add` action would make the main money amount greater than `999,999.99$`, the website should do nothing: no message, no saved data change, no `Balance Changes` entry, and the same money amount input step stays open. Money amounts up to `999,999.99$` should stay editable in the horizontal input square with required comma separators while typing and readable on the dashboard with required comma separators after saving, without horizontal page overflow, overlapping other content, or hiding actions. Under the square, the website should show `Save Changes`, with buttons exactly named `Yes` and `Cancel`. `Yes` should try to apply the selected action, `Cancel` should close without changing anything, and clicking or tapping outside should act like `Cancel`.
