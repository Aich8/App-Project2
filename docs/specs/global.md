...
Main money action inputs for `Add`, `Subtract`, and `Modify` should open as a horizontal money amount input square in the middle of the screen, with the main money amount circle still visible in the background. The square should start by showing `0.00` without the `$` sign. The user should type only the money amount, without the `$` sign. Main money amount inputs should request a mobile keyboard suitable for digit entry on supported devices. The user should enter cents in main money amount inputs with `Space`, not by typing a decimal point.

Main money amount inputs should accept only numbers from `0` through `9` and the `Space` key. Typed decimal points, comma separators, `$` signs, letters, minus signs, starting `Space`, consecutive `Space`, additional `Space` after two accepted `Space` separators, a third cents digit after the cents group is fixed by two accepted `Space` separators, paste, and other blocked input should not change the field and should show no message. The decimal point and comma separators in the displayed input should be generated by the website only.

For accepted input without `Space`, all digits are whole money amount digits: typing `5` should show `5.00`, `58` should show `58.00`, `589` should show `589.00`, `5895` should show `5,895.00`, `58955` should show `58,955.00`, and `589550` should show `589,550.00`. `0` is a normal digit, not a starting zero-position skip, and unneeded leading zeros in the whole money amount should be normalized away, so typing `0005` should show `5.00`.

The `Space` key should act as a non-visible separator between digit groups. `Space` should be accepted only after at least one digit and only when the previous accepted character is a digit. If the input ends with `Space`, all completed digit groups should be treated as the whole money amount and cents should show as `00`. If the final group after a `Space` has one or two digits, that final group should be treated as cents and should be left-padded with `0` when it has one digit. All earlier groups should be joined together as the whole money amount. If there is only one accepted `Space` and the final group grows to three or more digits, that final group should be treated as another whole money amount group and cents should show as `00`. Once the input has two accepted `Space` separators, the final group is the cents group and should accept at most two digits. Additional `Space` key presses after two accepted `Space` separators should be blocked with no message.

Examples: typing `5`, then `Space`, keeps the display at `5.00`; typing `5`, then `Space`, then `5` shows `5.05`; typing `5`, then `Space`, then `50` shows `5.50`; typing `58`, then `Space`, then `430` shows `58,430.00`; typing `58`, then `Space`, then `430`, then `Space`, then `88` shows `58,430.88`; typing `0`, then `Space`, then `5` shows `0.05`; typing `999999`, then `Space`, then `99` shows `999,999.99`; and typing `Space`, `Space`, `Space`, then `5` blocks the three `Space` key presses and then shows `5.00` after the `5` is typed.

If the user deletes one typed character, the website should remove the last accepted digit or accepted `Space`, then format the remaining accepted input again with two digits after the decimal point, automatic comma separators when needed, and no `$` sign. If the user deletes all typed numbers, the square should return to `0.00` and should not become empty.

The main money amount input is append-only: focus is allowed, but users should not move the cursor into the formatted value, select part of it, replace selected text, or edit generated comma separators or the generated decimal point. Accepted typing goes to the end, and delete removes only the last accepted digit or accepted `Space`.
...
`Modify` silently corrects the current money amount when the money amount is from `0.00$` through `999,999.99$`. It should allow a corrected money amount from `0.00$` through `999,999.99$`, should not allow values above `999,999.99$`, and should not create history or notification entries. If `Modify` changes the main money amount to `0.00$`, the next click on the main money amount should show `Add` and `Modify`, but not `Subtract`. If `Modify` changes the main money amount to `999,999.99$`, the next click on the main money amount should show `Subtract` and `Modify`, but not `Add`.
